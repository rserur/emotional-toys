...
// hrm_wavelet_clean_v02.ino and averages spikes in the input signal instead
// of clipping.
...
const float LOW_CUTOFF = 0; // No frequencies below this
...
//    Serial.println(rawInput.getValue(rawInput.getLength() -1));
  if(rawInput.getLength() == WINDOWLENGTH) {
    ...
    // rate can either be sent directly to output, or used to increase/ decrease
    // the previous rate by 1
...
//    Serial.print("freq: "); Serial.println(freqs[i]);
//    if(i != 1) {
...
//    Serial.print("power: "); Serial.println(powers[i]);
  }
}
...
int getRate() {
  ...
  for(int i=1; i<=LEVELS; i++) {
    if(powers[i] + powers[i-1] != 0) {
      numerator += ((power(freqs[i],3) - power(freqs[i-1],3))/3)*((powers[i] - powers[i-1])/freq_widths[i-1]) + ((power(freqs[i],2) - power(freqs[i-1],2))/2)*(powers[i-1] - freqs[i-1]*((powers[i] - powers[i-1])/freq_widths[i-1]));
...
//    Serial.print("D: "); Serial.print(freq_widths[i]/2.0); Serial.print(" * "); Serial.println(powers[i] + powers[i-1]);
